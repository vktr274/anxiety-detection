...
def label_anxiety(verbio_path: str) -> None:
    ...
    for target_dir in target_dirs:
        ...
        if not os.path.exists(report_path):
            logging.warning(f"Path {target_path} does not have a Self_Reports directory.")
            continue
        ...
        for file in next(os.walk(report_path))[2]:
            ...
            formatted = questionnaires[['PID', 'STAI State Score']].rename(columns={'PID': 'pid', 'STAI State Score': 'stai_score'})
            ...
            formatted.to_csv(os.path.join(output_path, f"{target_dir}_formatted.csv"), index=False)


if __name__ == "__main__":
    parser = ArgumentParser(description="Script for labeling anxiety using the STAI questionnaire scores in the VerBIO dataset.")
